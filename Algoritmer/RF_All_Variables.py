...
#Input: A df and colum name for column where value is like: 29% (str)
#Output: A df where the column value is: 29 (int)
def remove_percentage_sign(df_in, col_name):
    df_out=df_in
    df_out[col_name] = df_out[col_name].str.replace('%', '')
...
def random_forrest_SMOTE(X_train, X_test, y_train, y_test):
    print("------------------------RF with SMOTE --------------------------")
    bsmote = BorderlineSMOTE(random_state = 42, kind = 'borderline-2', k_neighbors = 2)
...
df_diagnos2=remove_percentage_sign(df_diagnoser,'Engelska totalt')
df_betyg_franvaro_diagnoser =df_betyg_franvaro.merge(df_diagnos2, how = 'inner', on = 'slumpkod')
...
# ----------------------------------- RF algorithm --------------------------------------------------------
rf_classifier_A = random_forrest(X_train, X_test, y_train, y_test)                # RF function that create, split, train and return statements about OOB and accuracy.
...
feature_imp = pd.Series(rf_classifier_A.feature_importances_, index=['Frånvarotid', 'Matematik',
...
rf_classifier_B = random_forrest_SMOTE(X_train, X_test, y_train, y_test)                  # RF function that create, train and return statements about OOB and accuracy AND apply SMOTE
...
feature_imp = pd.Series(rf_classifier_B.feature_importances_, index=['Frånvarotid', 'Matematik',
                            'page_view_factor', 'participation_factor',
                         'missing_factor','late_factor','on_time_factor','Engelska totalt',
                        'Svenska rättstavning', 'Svenska bokstavskedjor Stanine',
                         'Svenska ordkedjor Stanine','Svenska meningskedjor Stanine'])
...
feature_imp = pd.Series(rf_classifier_1.feature_importances_, index=['Frånvarotid', 'Matematik',
                            'page_view_factor', 'participation_factor',
                         'missing_factor','late_factor','on_time_factor','Engelska totalt',
                        'Svenska rättstavning', 'Svenska bokstavskedjor Stanine',
                         'Svenska ordkedjor Stanine','Svenska meningskedjor Stanine'])
